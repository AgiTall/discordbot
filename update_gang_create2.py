...
if "GangCreateConfirmView" not in text:
    text = text.replace("class GangsCog(commands.Cog):", view_code)

def replace_gang_create(m):
    return """    @app_commands.command(name="gang-create", description="Создать банду (Цена: 50 Золота)")
...
text = re.sub(r"    @app_commands\.command\(name=\"gang-create\".*?reset_economy_guild_id\(token\)", replace_gang_create, text, flags=re.DOTALL)
